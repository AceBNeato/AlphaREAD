...
export function LevelLongVowels({ levelId, accent }: LevelLongVowelsProps) {
  ...
  const handleItemClick = (item: string) => {
    ...
    if (patternData) {
      ...
      playExclusiveAudio(`${(import.meta as any).env.BASE_URL}audio/long-vowels-audio/name-${vowel}.mp3`).catch(() => {});
    } else {
      // It's a word or something else, try specific audio, then fallback to TTS
      playExclusiveAudio(`${(import.meta as any).env.BASE_URL}audio/long-vowels-audio/${item.toLowerCase()}.mp3`).catch(() => {
