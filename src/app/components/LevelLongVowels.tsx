...
import { LevelLongVowelsSentences } from "./LevelLongVowelsSentences";
import { LevelReviewGrid } from "./LevelReviewGrid";
import { LevelListenType } from "./LevelListenType";
import { supabase } from "../../lib/supabase";
import { useNavigate } from "react-router";
import { SyllableTarget, LONG_VOWELS_DATA, shuffle, getPhoneticPronunciation } from "../data/levels";
...
function MatchPhase({
  ...
  const playAudio = (word: string) => {
    if ("speechSynthesis" in window) {
      ...
      const phonetic = getPhoneticPronunciation(word.toUpperCase(), "LONG_VOWEL" as any);
...
export function LevelLongVowels({ levelId, accent }: LevelLongVowelsProps) {
  ...
  // Dynamically generate a single random pool of 10 long vowel words each time the level starts!
  const STEPS: GameStep[] = useMemo(() => {
    const allWordsRaw: string[] = [];
    LONG_VOWELS_DATA.forEach((d) => {
      d.patterns.forEach((p) => {
        p.words.forEach((w) => {
          allWordsRaw.push(w.word);
        });
      });
    });
    
    const randomWords = shuffle([...allWordsRaw]).slice(0, 10);
      ...
      <div className="sticky top-0 z-20 bg-white/80 dark:bg-[#0d141c]/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 px-4 py-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-3 w-full">
          ...
          <h2 className="text-lg font-bold tracking-tight" style={{ color: accent.primary }}>
            Long Vowels {step.phase === "review" ? "Review" : step.phase === "build" ? "Builder" : step.phase === "match" ? "Match" : step.phase === "eval" ? "Voice Eval" : "Type"} {step.setLabel ? `(${step.setLabel})` : ""}
          </h2>
