...
import { LONG_VOWELS_SENTENCES } from "../data/levels";
import { useSpeechRecognition } from "../hooks/useSpeechRecognition";
import { playSound } from "../utils/soundEffects";

interface LevelLongVowelsSentencesProps {
  levelId: number;
...
const totalSets = Math.ceil(LONG_VOWELS_SENTENCES.length / SENTENCES_PER_SET);

export function LevelLongVowelsSentences({ levelId, accent, isSubPhase, onComplete }: LevelLongVowelsSentencesProps) {
  ...
  useEffect(() => {
    setActiveSentences(LONG_VOWELS_SENTENCES.slice(currentSetIndex * SENTENCES_PER_SET, (currentSetIndex + 1) * SENTENCES_PER_SET));
      ...
      if (profileStr) {
        ...
        if (profile.id) {
          await supabase.from("progress").insert({
            ...
            level_id: levelId,
            score: LONG_VOWELS_SENTENCES.length,
          });
        }
      }
    } catch (err) {
