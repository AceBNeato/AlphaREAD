    ...
    <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="flex flex-col items-center w-full">
      ...
      <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-6 max-w-xl mx-auto px-2">
        {[...typeOrder].sort().map((item) => (
          <span
            key={`ref-${item}`}
            className="px-3.5 py-1.5 sm:px-5 sm:py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200 rounded-xl font-black text-base sm:text-lg flex items-center justify-center min-w-[3rem] sm:min-w-[4rem] select-none cursor-pointer btn-3d-effect"
          >
      ...
      <div className="flex justify-center gap-3 sm:gap-6 w-full max-w-2xl mx-auto mb-4 px-2 sm:px-4">
            ...
            return (
              <div key={`input-${item}`} className="relative w-full h-12 sm:h-14">
                <input
                  ...
                  disabled={isCorrect}
                  className={`w-full h-full text-center font-black text-xl sm:text-2xl tracking-widest rounded-xl border-[3px] outline-none transition-all shadow-sm ${
                    isCorrect
