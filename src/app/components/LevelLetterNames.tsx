...
import { allLetters as ALL_LETTERS, LETTER_TTS } from "../data/levels";
import { motion, AnimatePresence } from "motion/react";
import { ChevronRight, Home, ArrowRight, ArrowLeft, SkipForward, Shuffle, Volume2 } from "lucide-react";
...
export function LevelLetterNames({ levelId, accent }: LevelLetterNamesProps) {
  ...
  const playNameTTS = (letter: string) => {
    ...
    if (!letter) return;
    playExclusiveAudio(`${(import.meta as any).env.BASE_URL}audio/letter-names/name-${letter.toLowerCase()}.mp3`);
  };
